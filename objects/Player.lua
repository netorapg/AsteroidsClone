...
function Player(debbuging)
    local SHIP_SIZE = 30
    local VIEW_ANGLE = math.rad(90)
    local LAZER_DISTANCE = 0.6 -- distance lazers can go before dissapearing
    local MAX_LAZERS = 10 -- max amount of lazers on screen

    debugging = deugging or false

    return {
        x = love.graphics.getWidth() / 2,
        y = love.graphics.getHeight() / 2,
        radius = SHIP_SIZE / 2,
        angle = VIEW_ANGLE, -- angle gets calculaed as radian
        rotation = 0,
        thrusting = false,
        ...
        thrust = {
            ...
            y = 0,
            speed = 5,
            big_flame = false, -- this will change the flame size
            flame = 2.0
        },
        ...
        -- this fucntion will allow us to shoot lazers
        -- hese functions can be in lazer as well, although 
        -- it can be easier to use inside player
        shootLazer = function (self)
            if (#self.lazers <= MAX_LAZERS) then
                -- lazer spawn from front os ship
        ...
        draw = function (self)
            ...
            if self.thrusting then --only show the flame whenever e thrust forwards
